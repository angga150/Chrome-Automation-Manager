...
test('e2e: CDP navigate and screenshot', async (t) => {
  try {
  ...
  t.after(async () => {
    ...
    try { rmSync(profileManager.getProfilePath(session), { recursive: true, force: true }); } catch {}
    try { rmSync(join(process.cwd(), `cdp-e2e-${port}.png`), { force: true }); } catch {}
  });
  ...
  const out = join(process.cwd(), `cdp-e2e-${port}.png`);
